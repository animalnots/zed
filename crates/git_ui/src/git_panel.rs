...
impl GitPanel {
        ...
        match &self.view_mode {
            GitPanelViewMode::Flat => {
                ...
                for ix in start..=end {
                    if let Some(path) = self.marked_entry_for_ix(ix)
                        && !self.marked_entries.contains(&path)
                    {
            ...
            GitPanelViewMode::Tree(state) => {
                ...
                if let Some((anchor_pos, target_pos)) = anchor_pos.zip(target_pos) {
                    ...
                    let raw_indices: Vec<usize> =
                        state.logical_indices[start..=end].to_vec();
                    for raw_ix in raw_indices {
    ...
    fn revert_selected(
        ...
        if self.marked_entries.len() > 1 {
            ...
            let prompt = prompt::<DiscardCancel>(
            ...
            cx.spawn_in(window, async move |this, cx| {
                if !matches!(prompt.await?, DiscardCancel::Discard) {
                    return anyhow::Ok(());
                }
                ...
                cx.listener(move |this, event: &ClickEvent, window, cx| {
                    if event.modifiers().shift {
                        ...
                        if this.marked_entries.is_empty()
                            && let Some(prev) = this.selected_entry
                            && let Some(path) = this.marked_entry_for_ix(prev)
                        {
